...
static void init_pcb(struct process_control_block *ps);
static void start_process(struct t_args *targs) NO_RETURN;
...
process_execute (const char *file_name)
{
    ...
    struct process_control_block *ps = malloc (sizeof (struct process_control_block));
    init_pcb(ps);
    struct thread *cur = thread_current();
    list_push_back(&(cur->child_list), &ps->elem);
    ...
    /* Make a copy of FILE_NAME to extract filename . */
    char *fn_copy = palloc_get_page (0);
    if (fn_copy == NULL)
        return TID_ERROR;
    strlcpy (fn_copy, file_name, PGSIZE);

    /* Make second copy of FILE_NAME for args pushing. */
    ...
    char *save_ptr;
    char *file_name_only = strtok_r(fn_copy, " ", &save_ptr);
    ...
    tid = thread_create (file_name_only, PRI_DEFAULT, start_process, targs);
    if (tid == TID_ERROR) {
        palloc_free_page(fn_copy);
        free(ps);
        return TID_ERROR;
...
start_process (struct t_args *targs)
{
    ...
    /* Calculate argc */
    const char **argv_tokens_arr = (const char**) palloc_get_page(0);
    int argc = 0;

    char* save_ptr;

    for (char *token = strtok_r(file_name, " ", &save_ptr); token != NULL; token = strtok_r(NULL, " ", &save_ptr)) {
        argv_tokens_arr[argc++] = token;  // Store each token pointer
    }
    argv_tokens_arr[argc] = NULL;  // Null-terminate
...
static void
push_arguments(const char *argv_tokens[], int argc, void **esp) {
...
{
    struct process_control_block *child = find_child (thread_current(), child_tid);
...
void
init_pcb (struct process_control_block *ps)
{
    ps->exited = false;
